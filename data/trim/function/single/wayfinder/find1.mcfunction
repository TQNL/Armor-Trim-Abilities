...
scoreboard players reset @s trim.locate_test
...
execute store success score @s trim.locate_test run data modify storage trim_abilities:wayfinder root.check_locate set value "locate"
execute if score @s trim.locate_test matches 1 run return run tellraw @s {"color":"red","text":"Invalid command given"}
